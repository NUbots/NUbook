...
var searchRecursive = function (dir, pattern) {
  //https://stackoverflow.com/a/47886550
  var results = []
...
for (var j = 0; j < files.length; j++) {
  ...
  for (var i in array) {
    ...
    while (found == reBrackets.exec(array[i])) {
      //gets text in brackets

      var t = found[1].split(' ', 1)[0]
      if (t.includes('/')) {
        //check if stuff in brackets contains a link & whether external/internal
        //count++;
        //listOfText.push(t); //holds the links in the page
        //console.log(t + " file" + j);
        if (t.charAt(0) == 'h') {
          //Put into list of only external links
