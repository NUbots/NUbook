...
Any points that are not goal points or are completely surrounded by goal points are discarded. Any points above the green horizon are also discarded. The points we have left are goal post edge points. These points are clustered. Any clusters that do not intersect the green horizon are discarded, since we assume goals are higher than the field. Any clusters that are smaller than a given threshold are also discarded.
