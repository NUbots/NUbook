...
All goal post edge points (points that are goal points that have at least one neighbour that is **not** a goal post) are found and partitioned into clusters. Clusters are **discarded** if they fail to meet any of the following criteria
