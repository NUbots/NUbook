...
The Visual Mesh is an input transformation that uses knowledge of a cameras orientation and position relative to an observation plane to greatly increase the performance and accuracy of a convolutional neural network. It utilises the geometry of objects to create a mesh structure that ensures that a similar number of samples points are selected regardless of distance. The result is that networks can be much smaller and simpler while still achieving high levels of accuracy. The Visual Mesh is very capable when it comes to detecting distant objects. Distant objects often become too small for a network to detect accurately, but as the Visual Mesh normalises, its size detections are still accurate. The Visual Mesh can detect soccer balls, field lines and goal posts.
