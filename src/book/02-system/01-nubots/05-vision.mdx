...
| ------------------- | -------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------- |
| `serial_number`     | string               | The serial number of the camera.                                                                                                                  |
| `lens.projection`   | string               | The fisheye lens projection type. Can be rectilinear, equidistant or equisold.                                                                    |
...
The [ball detector receives](https://github.com/NUbots/NUbots/blob/master/module/vision/BallDetector/) a GreenHorizon message and a [FieldDescription message](https://github.com/NUbots/NUbots/blob/master/shared/message/support/FieldDescription.proto). The FieldDescription message is output by the [SoccerConfig module](https://github.com/NUbots/NUbots/tree/master/module/support/configuration/SoccerConfig) which creates the FieldDescription message from configuration values specifying the layout of the soccer field.
...
Clusters are **discarded** if they fail to meet any of the following criteria

- The cluster is below the field edge (i.e. the green horizon).

- The cluster is smaller than a given threshold.

Each remaining cluster is fitted with a **circular cone**. The **cone axis** is determined from the line segment between the centre on the camera and the average of all ball edge points. The radius of the cone is determined by the maximum distance between the ball edge points and the average of all of the ball edge points.

Clusters are **discarded** if they fail to meet any of the following criteria

- The cluster fits the **shape of a circle** well enough to be a ball. We use degree of circle fit to determine this. This involves calculating the standard deviation of the angle between all the rays in the cluster. The standard deviation must not exceed a given threshold.

  $$
  \text{angle}_i = \frac{\text{axis}_i \cdot \text{ray}_i}{ \text{radius}_{\text{max}}}
  $$
  ...
  - $\text{axis}_i$ is the cone axis as specified above of the $i$th cluster
  - $\text{ray}_i$ is the ray vector of the $i$th cluster
...
- The cluster must have distance less than the length of the field, as given by the FieldDescription message.
...
Points are **discarded** if they fail to meet any of the following criteria

- The point is labelled as a goal point by the Visual Mesh

- The point is **not** surrounded by other goal points

- The point is below the green horizon

The points we have left are goal post edge points. These points are **clustered**.
