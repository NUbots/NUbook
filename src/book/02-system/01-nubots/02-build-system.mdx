...
- Python3: A `requirements.txt` file exists in the root of the NUbots repository and in the `nuclear` folder. Install the required python modules using `sudo -H pip3 install -r requirements.txt` and `sudo -H pip3 install -r nuclear/requirements.txt`
- Visual Studio Code: recommended, but not required. Visual Studio code also has a plugin to allow you to develop inside the Docker container (only using the `generic` image).
...
As one might expect, `./b build` will build all enabled roles and required dependencies using `ninja`.
...
Be sure that you are using the `generic` target for this (`./b target generic`). You will likely want to recompile your role after setting `CMAKE_BUILD_TYPE` to either `Debug` or `RelWithDebInfo` otherwise you will likely get useless information.
...
| Subcommand     | Description                                                                              |
| -------------- | ---------------------------------------------------------------------------------------- |
| --asan         | Enables address sanitisation, a fast memory error detector.                              |
...
Currently, using the `--asan` option adds an environment variable that will direct ASan to log all of its output to a file. This is done to ensure that even when you are running a curses-enabled role (e.g. `keyboardwalk`) you will always be able to revocer the output from ASan (and it will also be clean from other log messages). There are a number of other run-time options you could set to tune the behaviour of ASan, they can be found [here](https://github.com/google/sanitizers/wiki/AddressSanitizerFlags). Just run your role as follows

```sh
ASAN_OPTIONS=option1=value1:option2=value ./b run --asan keyboardwalk
```

<Alert type="info">

If you would like to use a different path for your ASan log file be sure to specify `ASAN_OPTIONS=log_path=path/to/log/file`. However, there are very few locations inside of docker that have persistent write access.
...
If you need to run GDB and valgrind simulataneously, have a look at [this page](https://www.valgrind.org/docs/manual/manual-core-adv.html#manual-core-adv.gdbserver).

</Alert>

<Alert type="info">

If you need help deciding between ASan and valgrind have a look at [this page](https://github.com/google/sanitizers/wiki/AddressSanitizerComparisonOfMemoryTools).
