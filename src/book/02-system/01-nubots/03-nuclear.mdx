...
The developer can define the callback which will execute when the reaction is triggered during runtime. The callback can be defined using a C++ lambda function.
...
| enable(bool set) | Sets the run status of the reaction handle. True will enable and false will disable the reaction.                                                                                                   |
| enabled()        | Informs if the reaction is currently enabled. Returns true if enabled, false if disabled.                                                                                                           |
| unbind()         | Removes a reaction request from the runtime environment. This action is not reversible, once a reaction has been unbound, it is no longer available for further use during the instance of runtime. |
