...
When measured relative to the robot the **x-axis** is **forward** out of the torso, the **y-axis** is to the **left** and the **z-axis** is **up** the neck. This is robot space, also called torso space.

World space is defined with the **z-axis** pointing **up**. The other two axes are determined from the directions of the x-axis and the y-axis of the robot when it turns on.

For measurements on the field we use field coordinates. For field coordinates the **x-axis** is **toward the opponent's goal**, the **y-axis** is **left** when facing the opponents goal and the **z-axis** is **up**.
...
![A soccer field with a marker in the middle of the left half of the field labelled 'robot'. An arrow pointing up from this marker is labelled 'y', and an arrow pointing towards the other side of the field is labelled 'x'. The left side of the field is labelled as our side, whereas the other is the opponent's side.](./images/field-axis.svg 'Coordinates relative to a robot on the field')
...
This assumes the original vector, `rCAa` **has already been rotated** into space $b$, ie we have `rCAb`.
...
If you would like the vector **only rotated**, extract the **rotational component** of the transform and apply it to the vector. This does not hold the same restriction as above, however the **spaces still need to match**.

```cpp
Eigen::Vector3d rCDb = Hba.rotation() * rCDa;
```

Homogeneous transformation matrices can be **multiplied** together to get a **new homogemeous transformation** matrix.
...
If you have two **vectors** in the **same space** and their **inner** (or **outer**, since vector addition is **commutative**) **letters** are both the same then they can be **canceled** in a similar way to the transformation matrices.
