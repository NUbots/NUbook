...
NUbots use a **right-handed coordinate system** where the x-axis is **forward**, the y-axis is to the **left** and the z-axis is **up**.

![A right hand with the index finger pointed forwards and labelled 'a', the middle finger pointing to the left and labelled 'b', and the thumb pointing upwards and labelled 'a x b'](./images/right-hand-rule.svg 'Right-Hand Rule. Image (modified) from Wikimedia Commons https://en.wikipedia.org/wiki/File:Right_hand_rule_cross_product.svg')
