...
The build pipeline is configured in the [`.buildkite/pipeline.yml`](https://github.com/NUbots/NUbots/blob/master/.buildkite/pipeline.yml) file in the NUbots repo. This file specifies a number of "build steps", which are separate jobs such as "Build for NUC" and "Validate C++ and Protobuf formatting". We edit this file to add, remove, or change build jobs.

The [Buildkite pipeline documentation](https://buildkite.com/docs/pipelines) has everything you need to know to make changes.
...
Credentials are provided to the agent via env variables from agent hooks system. Specifically via the environment hook. The default location for this hook is `/etc/buildkite-agent/hooks/environment`. The default path for the agent's configuration file is `/etc/buildkite-agent/buildkite-agent.cfg`.
