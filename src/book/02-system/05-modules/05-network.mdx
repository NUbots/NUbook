...
This module allows for sending data from NUbots to [PlotJuggler](https://github.com/PlotJuggler/PlotJuggler) for plotting in real time. It does this by listening for `DataPoint` messages (what we use for plotting in NUsight), transforming them to a suitable format and sending them to PlotJuggler via UDP.
