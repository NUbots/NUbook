...
In the `READY` mode, the player will dynamically position based on the number of team mates on each side. One robot (closest to the field centre on the left side) will position either inside or just outside of the centre circle for kick off.
