...
This implementation of CycleGAN uses real images and NUpbr paired raw and segmentation images. The NUpbr raw images represent the A domain of the CycleGAN. The real images represent the B domain of the CycleGAN. We can then pair up our fake images with NUpbr segmentation images to obtain a dataset of pseudo-real images. 
...
Since we do not hve an attention mask for images in the B domain, we focus more on cycling on the original real A image. From a real B image, we generate a fake A image. We do not cycle from here. From a real A image, we cycle on the resulting fake B image, and then again on the resulting cycled fake A image. The cycles from the real A image all have an attention mask from the corresponding segmentation mask for A. 
