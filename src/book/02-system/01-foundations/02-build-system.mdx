...
| doc       | Create json documenting each module's reactors                 |
...
The `doc` command of `b` generates a JSON file documenting each Module. It lists each reactor, with their on statements and emit statements. The on and emit statements have their location in the file and their dsl worlds also.

The `--indir` argument gives the root of the directories that will be documented, defaults to the `./module/` directory.

The `--outdir` argument gives the directory that the JSON output will be written, defaults to `./doc/NUdoc/` directory.

The `-m` or `--multiprocess` argument tells the command to use multiprocessing to greatly reduce the processing time, but uses a very large amount of RAM, about 1G per thread.
