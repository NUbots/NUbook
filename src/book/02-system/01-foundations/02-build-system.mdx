...
NUClear Roles provides special wrappers for certain types. Vector and matrix types are mapped to [Eigen](https://libeigen.gitlab.io/eigen/docs-nightly/index.html) types. To use these mappings simply use `import Vector.proto;` and `import Matrix.proto;`. Some examples of types that you can use are:
...
| retrieve  | For retrieving recordings, configs, and scripts from the robot |
...
To get data back from a robot (whether this may be configs, recordings, or scripts), use the `retrieve` command of `b` in any of the following ways

```bash
./b retrieve n1 recordings # retrieve recordings from 10.1.1.1
./b retrieve 10.1.1.67 config # retrieve configs from 10.1.1.67
./b retrieve nugus1 scripts # retrieve scripts from nugus1
./b retrieve hostname all # retrieve everything from a hostname
```

`retrieve` takes two positional arguments, `host` then `target`, plus one optional flag.

| Argument/Flag  | Description                                                                                                                                      |
| -------------- | ------------------------------------------------------------------------------------------------------------------------------------------------ |
| `host`         | The robot to retrieve from. Accepts `n1`-`n4`/`nugus1`-`nugus4`/`i1`-`i4`/`igus1`-`igus4` (mapped to `10.1.1.1`-`10.1.1.4`) or a raw hostname/IP |
| `target`       | What to retrieve: `config`, `recordings`, `scripts`, or `all`                                                                                    |
| `--user`, `-u` | The user to retrieve the files with. Defaults to `nubots`                                                                                        |

Depending on `target`, this script copies the following back into your local codebase, ready for committing to our GitHub.

- `recordings`: copied to `recordings/`

- `config`: scanned for whether they are global or specific to a robot, then sent to the correct folder

- `scripts`: also scanned for locality then sent to the correct place

- `all`: does all three of the above

<Alert type="warning">

Recordings can be absolutely massive files.

</Alert>

##### Blackbox

Should you want to see a black-box-ified version of a module, you can `blackbox`.
It will generate a nice little diagram for you with all the NUClear dependencies, like the `.yaml` file on which it configures, the modules that it will react to, the protobufs it _can_ emit, and the other outputs it is configured for, such as PlotJuggler, or data streaming, like `NatNet`.

```bash
./b blackbox FieldLocalisationNLopt
```

This command will spit out the following image to `/recordings/blackbox`:

![FieldLocalisationNLopt as a black-box module](./images/localisation_FieldLocalisationNLopt.png)

The following syntax is also valid:

Specify the specific folder the module resides in:

```bash
./b blackbox localisation/FieldLocalisationNLopt
```

Specify two or more modules to generate a drawing of:

```bash
./b blackbox Camera NatNet
```

Generate a list of all the modules you could get a drawing of:

```bash
./b blackbox --list
```

The diagram also shows you the datatypes in each file and protobuf, and will also show you whether it accesses the field in the protobuf; greying out the field if it is not accessed.
<Alert type="warning">

The method it uses to assess whether a protobuf field is accessed is an educated guess. It has a minute false-positive rate and should not be trusted as the be-all-and-end-all.
...
When `USE_ASAN=ON` an environment variable is added that will direct ASan to log all of its output to a file. This is done to ensure that even when you are running a curses-enabled role (e.g. `fake/keyboardwalk`) you will always be able to recover the output from ASan (and it will also be clean from other log messages). There are a number of other run-time options you could set to tune the behaviour of ASan, they can be found [here](https://github.com/google/sanitizers/wiki/AddressSanitizerFlags). Just run your role as follows
...
It's recommended to add it to your `.bashrc`, but you need to provide the full path to the NUbots folder. e.g. `eval "$(register-python-argcomplete $HOME/code/NUbots/b)"`.
...
Two images are created, one intended for running code on your local machine, named `generic`, and another intended for running code on the robots named `nuc12wshi7`.
...
A list of the currently used compiler flags for the `nuc12wshi7` can be seen in [generate_nuc12wshi7_toolchain.py](https://github.com/NUbots/NUbots/blob/main/docker/usr/local/toolchain/generate_nuc12wshi7_toolchain.py).
...
CMake needs to know what dependencies we have and where to find them.
In a module's folder the `CMakeLists.txt` file tells CMake what is needed to build a module.
The `nuclear_module()` function tells CMake that this directory contains a module that may be added to a role.
This function may take named parameters.

| Parameter  | Usage                                                                |
| ---------- | -------------------------------------------------------------------- |
| LANGUAGE   | The programming language that this module is in (currently only C++) |
| LIBRARIES  | The libraries that this module depends on and must be linked with    |
| SOURCES    | Extra code outside the `src` directory that must be built            |
| DATA_FILES | Extra data files outside the `data` directory that the module uses   |
...
CMake can also look for build instruction files in `/lib/cmake/X/XConfig.cmake` where `X` is the name of the library.
An example of one of these files, `FindAravis.cmake` which finds the Aravis library, is shown below
...
The first line includes a custom function for later use.
We have `find_package` again, as this library has its own dependency.
Then there is `ToolchainLibraryFinder` which is the function that finds the library.
Finally there is `target_link_libraries` which tells CMake that we have a dependency of the library.
`ToolchainLibraryFinder` parameters:

| Parameter                | Usage                                                                                                                                                                                                                               |
| ------------------------ | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| NAME                     | The name we use for the package                                                                                                                                                                                                     |
| HEADER                   | Any header, e.g. `.h` or `.hpp`, files that define the api of the library                                                                                                                                                           |
| LIBRARY                  | The name of the library on disk, e.g. if we have `/usr/lib/libaravis-0.8.so` it'd be `aravis`                                                                                                                                       |
| LIBRARIES                | A list instead of a single LIBRARY                                                                                                                                                                                                  |
| PATH_SUFFIX              | Add subdirectories to look for includes and libraries in the normal search directories                                                                                                                                              |
| BINARY                   | A binary that can be used to extract the version number                                                                                                                                                                             |
| VERSION_BINARY_ARGUMENTS | The arguments to pass to the binary that will result in a version number being printed                                                                                                                                              |
| VERSION_FILE             | A file that contains information on the version number of the library                                                                                                                                                               |
| VERSION_REGEX            | The regex used on the version file to extract the version number                                                                                                                                                                    |
| LINK_TYPE                | The type of linkage that the library will use: `UNKNOWN` is the default and will pick either, `SHARED` will use dynamic linkage and look for a `.so` library file `STATIC` will use static linkage and look for a `.a` library file |
