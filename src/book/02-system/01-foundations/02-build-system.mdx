...
The `nuclear_modlue()` function tells CMake that this directory contains a module that may be added to a role.
...
| DATA_FILES | Extra data files outside the `data` directory that is necessary for the module |

Now for libraries in particular CMake needs to know where the libraries are located,
the function `find_package()` is used to tell CMake to find the library.

CMake then looks in the `cmake/Modules/` directory for files like `FindX.cmake` for the logic to find the files.
CMake can also look for build instruction files in `lib/cmake/X/` like `XConfig.cmake` where `X` is the name of the
library.
...
| LINK_TYPE                | The type of linkage that the library will use: `UNKNOWN` is the default and will pick either, |
|                          | `SHARED` will use dynamic linkage and look for a `.so` library file                           |
|                          | `STATIC` will use static linkage and look for a `.a` library file                             |
