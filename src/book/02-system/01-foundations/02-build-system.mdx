...
The `blackbox` command will generate a blackbox diagram of a module in the codebase.  A blackbox diagram will display all NUClear dependencies for the module, such as the associated `.yaml` configuration file, the modules that it will react to, the protobufs it _can_ emit, as well as PlotJuggler and NatNet packets.
