...
NBS files can have an acsossiated index file. This allows for quicker random access to the coresponding nbs file. An `nbs.idx` file has the folowwing frames repeated continuously, which are then compressed with gzip:

| Name      | Type   | Description                                                                      |
| --------- | ------ | -------------------------------------------------------------------------------- |
| hash      | uint64 | the 64bit hash for the payload type                                              |
| subtype   | uint32 | the id field of the message if it exists, otherwise 0                            |
| timestamp | uint64 | timestamp field of the message if it exists or the emit timestamp in nanoseconds |
| offset    | uint64 | offset to start of radiation symbol ☢                                            |
| size      | uint32 | Size of the whole packet from the radiation symbol                               |

This index file is also little enidan.
