...
![A collection of Reactors around the PowerPlant in the centre. Double ended arrows point to and from the PowerPlant to each Reactor.](images/power_plant.svg 'PowerPlant with Reactors. Image: https://nuclear.readthedocs.io/en/latest/components.html')
...
Looking at [`SensorFilter.cpp`](https://github.com/NUbots/NUbots/blob/5bb3e76d2a32d9e9f4367fb25b3338f032727c51/module/input/SensorFilter/src/SensorFilter.cpp) we can see some examples of NUClear on statements.

The first on statement is located at [line 94](https://github.com/NUbots/NUbots/blob/5bb3e76d2a32d9e9f4367fb25b3338f032727c51/module/input/SensorFilter/src/SensorFilter.cpp#L91) of `SensorFilter.cpp`.
...
The DSL request is `on<Configuration>`. This means the runtime argument here is `("SensorFilter.yaml")`. This is the [configuration file](https://github.com/NUbots/NUbots/blob/5bb3e76d2a32d9e9f4367fb25b3338f032727c51/module/input/SensorFilter/data/config/SensorFilter.yaml). The callback is
...
The `const Configuration& config` is a parameter of type `Configuration`. We can see examples of how this type works in the lines after it. `cfg["buttons"]["debounce_threshold"].as<int>()` finds the value labelled as [["buttons"]["debounce_threshold"] in the configuration file](https://github.com/NUbots/NUbots/blob/5bb3e76d2a32d9e9f4367fb25b3338f032727c51/module/input/SensorFilter/data/config/SensorFilter.yaml#L4) and retrieves it as an `int` type.

This value is then stored in the [member variable](https://github.com/NUbots/NUbots/blob/5bb3e76d2a32d9e9f4367fb25b3338f032727c51/module/input/SensorFilter/src/SensorFilter.hpp#L126) `debounceThreshold` in the struct `buttons`.

This reaction is used at startup to set many variables from a configuration file. The [next on statement](https://github.com/NUbots/NUbots/blob/5bb3e76d2a32d9e9f4367fb25b3338f032727c51/module/input/SensorFilter/src/SensorFilter.cpp#L188) does this as well, but with the [`FootDownNetwork.yaml`](https://github.com/NUbots/NUbots/blob/5bb3e76d2a32d9e9f4367fb25b3338f032727c51/module/input/SensorFilter/data/config/FootDownNetwork.yaml) configuration file.

The [third on statement](https://github.com/NUbots/NUbots/blob/5bb3e76d2a32d9e9f4367fb25b3338f032727c51/module/input/SensorFilter/src/SensorFilter.cpp#L193) uses the DSL words `Last`, `Trigger`, `With` and `Single`.
...
The DSL request is `on<Last<20, Trigger<RawSensors>>, With<KinematicsModel>, Single>`. This says that we will create a task for this reaction when we receive a [`RawSensors`](https://github.com/NUbots/NUbots/blob/5bb3e76d2a32d9e9f4367fb25b3338f032727c51/shared/message/platform/RawSensors.proto) message. The last 20 received `RawSensors` messages will be stored in a `std::list`. `With<KinematicsModel>` will get the last available [`KinematicsModel`](https://github.com/NUbots/NUbots/blob/5bb3e76d2a32d9e9f4367fb25b3338f032727c51/shared/message/motion/KinematicsModel.proto) message. The `KinematicsModel` message is emitted once `on<Configuration>` in the [KinematicsConfiguration module](https://github.com/NUbots/NUbots/blob/5bb3e76d2a32d9e9f4367fb25b3338f032727c51/module/motion/KinematicsConfiguration/src/KinematicsConfiguration.cpp) to provide measurements for the robot. Finally, the `Single` word is used to only run one task for this reaction at any given time. If a `RawSensors` message is received while a task is already running, a new task will **not** be created.
...
The [next on statement](https://github.com/NUbots/NUbots/blob/5bb3e76d2a32d9e9f4367fb25b3338f032727c51/module/input/SensorFilter/src/SensorFilter.cpp#L339) in `SensorFilter.cpp` uses `Trigger`, `Optional`, `With`, `Single`, and `Priority`.
...
Another aspect of this reactor is the `update_loop` variable. This is a ReactionHandle type and can be found in the [header file](https://github.com/NUbots/NUbots/blob/5bb3e76d2a32d9e9f4367fb25b3338f032727c51/module/input/SensorFilter/src/SensorFilter.hpp#L179). The reaction can be enabled by calling [`update_loop.enable()`](https://github.com/NUbots/NUbots/blob/5bb3e76d2a32d9e9f4367fb25b3338f032727c51/module/input/SensorFilter/src/SensorFilter.cpp#L276) and disabled by calling [`update_loop.disable()`](https://github.com/NUbots/NUbots/blob/5bb3e76d2a32d9e9f4367fb25b3338f032727c51/module/input/SensorFilter/src/SensorFilter.cpp#L184).

As expected, our callback has `const RawSensors& input` and `const KinematicsModel& kinematicsModel` since we asked for the `RawSensors` and the `KinematicsModel` messages. The interesting part of this is the `std::shared_ptr<const Sensors> previousSensors`. This will either be a `nullptr` or a valid Sensors message. [There are checks in the function to see if the pointer is null](https://github.com/NUbots/NUbots/blob/5bb3e76d2a32d9e9f4367fb25b3338f032727c51/module/input/SensorFilter/src/SensorFilter.cpp#L408) and this determines how the function behaves.

Looking at line 133 of [`ScriptTuner.cpp`](https://github.com/NUbots/NUbots/blob/105b78ad6655801f5f74d029cbd169ba8e8cb5e8/module/behaviour/tools/ScriptTuner/src/ScriptTuner.cpp#L133) we can see an example of an `on<IO>` reaction.
...
An example of local emission can be seen in [`SensorFilter.cpp`](https://github.com/NUbots/NUbots/blob/5bb3e76d2a32d9e9f4367fb25b3338f032727c51/module/input/SensorFilter/src/SensorFilter.cpp#L788) where we call

```cpp
emit(std::move(sensors));
```

This emits a [`Sensors`](https://github.com/NUbots/NUbots/blob/5bb3e76d2a32d9e9f4367fb25b3338f032727c51/shared/message/input/Sensors.proto) message. This can then be received by reactors waiting for a `Sensors` message to trigger the reaction.

Looking at line 236 of [`GameController.cpp`](https://github.com/NUbots/NUbots/blob/e7db7154a689ab076041375894b3baf1d53a2587/module/support/NUsight/src/GameController.cpp#L236), we can see an example of NUClear network emission.

```cpp
powerplant.emit_shared<Scope::NETWORK>(std::move(gameState), "nusight", true);
```

This tells the PowerPlant to emit something across the network. In this case, we are emitting `gameState`, which is a [`GameState`](https://github.com/NUbots/NUbots/blob/e7db7154a689ab076041375894b3baf1d53a2587/shared/message/input/GameState.proto) message. The target is `nusight`, signifying that it will be received by NUsight. `true` signifies that it is reliable.

Looking at line 112 of [`ScriptTuner.cpp`](https://github.com/NUbots/NUbots/blob/e7db7154a689ab076041375894b3baf1d53a2587/module/behaviour/tools/ScriptTuner/src/ScriptTuner.cpp#L112), we can see an example of an `INITIALIZE` scoped emit statement.
...
This tells the PowerPlant to emit something at system initialisation. In this case, we are emitting a [`RegisterAction`](https://github.com/NUbots/NUbots/blob/e7db7154a689ab076041375894b3baf1d53a2587/shared/utility/behaviour/Action.h#L34) which registers the `ScriptTuner` module with the subsumption system.
