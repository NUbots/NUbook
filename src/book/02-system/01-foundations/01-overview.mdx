...
| `headbehtest` | Real robot sensors and camera, vision modules, script modules, head controller and behaviour | Used to test the behaviour of the head, particularly with balls and goals. |
...
| `keyboardwalkfake` | As above but with a basic simulated robot (not webots) | Used for testing the code without a real robot, potentially to make sure there are no runtime errors. Used to demonstrate basic functionality of the robot. Part of the [Getting Started](/guides/main/getting-started) example since it does not require a real robot. |
| `localisationtest` |  Localisation, localisation simulator and NUsight | Used to test localisation in a simulated environment. |
| `lookatball` | Real robot sensors, vision and motions | Used to test head behaviour where just the ball is the focus. By removing robot localisation, just the ball should be looked at. |
| `lookatgoal` | Real robot sensors, localisation, vision and motions | Used to test head behaviour with localisation, which should make it look at the goals. |
...
| `nusighttest` | NUsight | Testing connection of NUsight, doesn't run anything that can send data. |
