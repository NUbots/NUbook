...
digraph {
    ...
    {"Servo State", "Acc/Gyro State"} -> "Hardware Input Module" -> "Forward Kinematics" -> "Odometry" -> {"Visual Mesh", "Walk Engine", "Localisation"}
    "Goal Detector" -> "Localisation" -> "Soccer Strategy"
    "Soccer Strategy" -> "Walk Path Planner" -> "Walk Engine"
    {"Ball Detector", "Localisation"} -> "Ball Localisation"
    {"Ball Detector", "Goal Detector", "Ball Localisation"} -> "Head Behaviour Soccer" -> {"Head Controller", "Walk Engine"} -> "Inverse Kinematics" -> "Servo Command"
}
```
