...
digraph {
    ...
    {"Servo State", "Acc/Gyro State"} -> "Hardware Input Module" -> "Odometry" -> {"Visual Mesh", "Walk Engine", "Localisation"}
    "Goal Detector" -> "Localisation" -> "Soccer Strategy"
    "Soccer Strategy" -> "Walk Path Planner" -> "Walk Engine" -> "Servo Command"
    {"Ball Detector", "Localisation"} -> "Ball Localisation"
    {"Ball Detector", "Goal Detector", "Ball Localisation"} -> "Head Behaviour Soccer" -> "Head Controller" -> "Servo Command"
}
```
