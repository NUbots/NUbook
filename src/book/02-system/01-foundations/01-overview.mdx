...
digraph {
    ...
    {"Servo State", "IMU State"} -> "Hardware Input" -> "Forward Kinematics" -> "Odometry" -> {"Visual Mesh", "Localisation"}
...
Other roles may only have a subset of the above diagram, for testing purposes. Some roles, such as `ps3walk` and `keyboardwalk` are also for demonstration purposes and bypass the autonomous planning sections of the system with user input from a PlayStation controller or keyboard input. 
