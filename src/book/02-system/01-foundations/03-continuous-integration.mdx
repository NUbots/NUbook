...
In order to cache these packages, a [pacman](https://wiki.archlinux.org/index.php/pacman) package cache is setup on each buildkite agent. This cache uses the [nroi/flexo](https://hub.docker.com/r/nroi/flexo) docker image, and exposes itself on

```
http://localhost:7878/$repo/os/$arch
```

In order for [pacman](https://wiki.archlinux.org/index.php/pacman) to use this package cache, the cache must be added to `/etc/pacman.d/mirrorlist`. Because our build happens inside a [buildx](https://docs.docker.com/buildx/working-with-buildx/) container, we cannot use `localhost` as the address for the cache. Instead, because the buildx container runs on the `bridge` network, we can ping the host system on `172.17.0.1`.
Therefore, the following is added too the mirrorlist:

```
Server = http://172.17.0.1:7878/$repo/os/$arch
```
