...
A Start Provider is used to set up the state of a Provider when it starts running. Start is executed when a Provider group has no active Tasks and then is given to one, i.e. it gains control.
...
These conditions are updated by using normal NUClear emits. For example if `emit(std::make_unique<Stability>(Stability::STATIC));` was emitted in another reaction, then the `When` condition would be met and the Provider would run.
...
Tasks can take four arguments `emit<Task>(data, priority, optional, name)`, as described in the following table.
...
The Root node represents a non-Provider reaction, eg an `on<Startup>()` reaction. It emits a `Play` Task with `emit<Task>(std::make_unique<Play>())`.
...
The Walk Provider has emitted a `LegIK` Task, but it is blocked because the Kick Provider has also emitted a `LegIK` Task. The Kick Provider has a higher priority, so it is the one that is currently active.
...
If the Kick Provider decides it no longer should Kick, such as if the ball is not in front of the robot, it might emit a `nullptr` to stop the Kick Provider. This will cause the Kick Provider to stop, and the Walk Provider to become active.

```cpp
on<Provide<Kick>>().then([this] (const Kick& kick) {
    if (kick.ball_in_front) {
        emit<Task>(std::make_unique<Kick>());
    }
    else {
        emit<Task>(std::make_unique<Kick>(nullptr));
    }
});
```

The following graph represents the case where the `else` statement runs.
