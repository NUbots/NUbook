...
For measurements on the field we use field space. For field coordinates the **x-axis** is **toward the opponent's goal**, the **y-axis** is **left** when facing the opponents goal and the **z-axis** is **up**. The origin of this space is the center of the field.
...
Let's say we have a collection of data points. We want to find a function that contains all these points. We call this function an interpolant. Often we use a polynomial to describe this function.

## Splines

Splines are used in the [Quintic Walk Engine](/system/subsystems/motion#walk-engine).

Rather than have one global polynomial, it can be useful to use piecewise polynomials. These are all defined locally around the interpolation points.
...
Every two consecutive points on the graph have an associated polynomial that is plotted in the range between these two points.
