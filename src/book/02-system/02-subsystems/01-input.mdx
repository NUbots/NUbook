...
These parameters are set for each camera as **configuration values** in the [Camera module](https://github.com/NUbots/NUbots/tree/main/module/input/Camera/data/config). Configuration files stored in the [config/Cameras directory](https://github.com/NUbots/NUbots/tree/main/module/input/Camera/data/config/Cameras) are common to all robots, and configs that are specific to a particular robot are located in that robot's respective directory. The values for the left camera on the robot will be stored in `Left.yaml`. The values for the right camera on the robot will be stored in `Right.yaml`.
