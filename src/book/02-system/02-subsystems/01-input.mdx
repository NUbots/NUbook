...
Input to the system includes cameras, [Game Controller](https://github.com/RoboCup-HumanoidSoccerLeague/GameController) and [NatNet](https://optitrack.com/products/natnet-sdk/).

## Cameras

The cameras used on the NUgus robot are [Flir Blackfly S USB3's](https://www.teledynevisionsolutions.com/en-au/products/blackfly-s-usb3).

The [Spinnaker SDK](https://www.teledynevisionsolutions.com/en-au/products/spinnaker-sdk/) is FLIR's Genicam API library which includes a GUI program called SpinView, used to operate the cameras from a PC, as well as extensive documentation.
...
| `lens.k`            | 2-dimensional vector | The polynomial distortion coefficients for the length                                                                                                          |
| `lens.fov`          | float (radians)      | Field of view. The angular diameter that the lens covers (the area that light hits on the sensor).                                                             |
| `roll_offset`       | float (radians)      | Roll of the camera in radians to account for physical differences from NUgus print manufacturing and assembly to align with the camera's virtual focal point.  |
| `pitch_offset`      | float (radians)      | Pitch of the camera in radians to account for physical differences from NUgus print manufacturing and assembly to align with the camera's virtual focal point. |

The following settings are applied to the cameras on startup. (Note: These are the camera settings we currently use, but there are many more. See the full <a href="#settings_list">list</a> at the bottom of this page.)
...
| `AutoExposureExposureTimeUpperLimit` | Float       | The largest exposure time that auto exposure can set                                                         |
| `AutoExposureTargetGreyValueAuto`    | Enumeration | Whether or not the target image grey level is automatically set by the camera.                               |
| `AutoExposureTargetGreyValue`        | Enumeration | Manual target grey level applied to the camera to compensate for light level.                                |
| `BalanceWhiteAuto`                   | Enumeration | Compensates for colour shifts caused by different lighting conditions.                                       |
| `BalanceRatioSelector`               | Enumeration | Set whether to balance red or blue.                                                                          |
| `BalanceRatio`                       | Float       | The ratio the camera will use for red or blue (set above) to balance for accurate colours.                   |
| `GainAuto`                           | Enumeration | Sets the automatic gain mode. In automatic modes, the camera adjusts the gain to maximise the dynamic range. |
...
| `DeviceLinkThroughputLimit`          | Integer     | Limits the maximum bandwidth on the USB bus. Can limit the framerate.                                        |

These parameters are set for each camera as **configuration values** in the [Camera module](https://github.com/NUbots/NUbots/tree/main/module/input/Camera/data/config). Configuration files stored in the [config/Cameras](https://github.com/NUbots/NUbots/tree/main/module/input/Camera/data/config/Cameras) directory are common to all robots, and configs that are specific to a particular robot are located in that robot's respective directory. The values for the left camera on the robot will be stored in `Left.yaml`. The values for the right camera on the robot will be stored in `Right.yaml`, however this is currently not used.
...
The [projection tool](https://github.com/NUbots/NUbots/tree/main/shared/utility/vision/projection.hpp), based on [panotools' fisheye projection calculations](https://wiki.panotools.org/Fisheye_Projection), maps a portion of the surface of a sphere to a flat image. The type of projection is specified by the above parameter `lens.projection`.

<details id="settings_list">

  <summary>Click here to see a full list of camera settings</summary>

**This file is a print out directly from the cameras using the [arv-tool-0.8](https://github.com/AravisProject/aravis) command line utility.**
This list was produced by running the command `arv-tool-0.8 features` with a camera connected to the computer via USB. Run `arv-tool-0.8 -h` for more command information.
