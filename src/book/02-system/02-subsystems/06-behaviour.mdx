...
Based on the specifications in the [RoboCup Humanoid Rules](https://github.com/RoboCup-Humanoid-TC/Rules) there are some states in the game where the robots must perform a particular behaviour differently to normal play. An example of this is when a penalty occurs in favor of the opposing team, which then recieves either a direct or indirect free kick, where the robots need to then act in a particular way such as standing still, or move out of the way for the duration of the kick.
