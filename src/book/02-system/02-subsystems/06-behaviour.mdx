...
When ScriptRunner is run, it stores the script file names passed to it as a vector of strings. When the middle button on the robot is pushed, the scripts are executed in order of appearance, one after the other. ScriptRunner also provides an option to set a delay before the execution of the first script, as well as a delay between the execution of each script.
...
To learn how to use ScriptTuner, read the [ScriptTuner guide](/guides/main/script-tuner).
