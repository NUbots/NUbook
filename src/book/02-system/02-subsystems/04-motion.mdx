...
The NUgus platform can be treated as a robotic manipulator consisting of rigid-body **links** connected by **joints**. Such a configuration is called a **kinematic chain**. The complete kinematic chain for the NUgus system is shown below. 

A **Forward Kinematic Model (FKM)** provides the position and orientation between two points on a chain given the value of robots joints in rads. For conventional reasons, the **joint frames** are orientated such that the direction of rotation always points in the joints z direction. 

![Screenshot of the NUgus forward kinematic model](./images/FKM.png 'FKM')

A FKM is constructed through the product of homogeneous transformations between joint frames. See the homogeneous transforms section here for more details.
...
Inverse kinematics is the process of determining the value of the joint variables given a desired position and orientation of the end effector.
