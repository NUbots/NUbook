...
where $W_n$ is the weight of the $n$th particle, $\delta = \sum_{i=1}^{k} \delta_i$ is the total sum of distance values obtained from the map for all field line point observations, and $\epsilon$ is machine epsilon to prevent numerical issues.
