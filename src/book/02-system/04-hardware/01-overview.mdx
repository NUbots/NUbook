...
- Redesigned the legs with a simpler model made out of aluminium, with XH540-W270-R servos rather than MX-106 servos, and onyx connectors for the hip and ankle blocks and onyx supports inside the legs.
...
- Added an aluminium bar between the hip yaw servos to reduce flex.
- Created custom draws for the computer (intel NUC) and subcontrollers (OpenCR, NUSense)

## Servos

The arms and head of the NUgus robot use [Dynamixel MX64AR servos](http://emanual.robotis.com/docs/en/dxl/mx/mx-64/), with two in each shoulder, one in each elbow, and two in the neck. The legs use [Dynamixel XH540-W270-R servo](https://emanual.robotis.com/docs/en/dxl/x/xh540-w270/), with two in each ankle, one in each knee, and three in each hip.
