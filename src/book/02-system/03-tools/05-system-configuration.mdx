...
After this the system will then attempt to "upgrade" the system using the package manager, up till the date docker uses.
This "upgrade" will actually be a downgrade if the robot is ahead of the date docker uses.
Once this is done it will go through the list of packages in configuration and ensure that they are installed.

### Systemd

The system will then go through the list of units and ensure that they are enabled.
...
[Zprezto](https://github.com/sorin-ionescu/prezto) is now installed and the appropriate symlinks are made.
The users' default shell is also changed to zsh.
