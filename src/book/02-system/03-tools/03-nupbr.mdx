...
The NUpbr repository can be found [within the NUbots organisation](https://github.com/NUbots/NUpbr). To run the data generation script, ensure [Blender 2.93 LTS](https://www.blender.org/download/lts/2-93/) is installed. Ensure that the configuration properties are established as desired, as found in `pbr/config/output_config.py`
This includes:
