...
Where `[options]` corresponds to various options as detailed in the [Build System](/system/nubots/build-system#install), and may be none, one or multiple options. `<robot>` corresponds to the name of the robot such as `n1` or `nugus1`.
...
Where `<address>` corresponds to `10.1.1.` followed by the robot number such as `1`, `2`, or `3`. Now you can run whatever you have built to the robot! For example, if you want to stand the robot up, run
...
Configuration (.yaml) files can be found in the config folder on the robot, and can be robot specific. These are in folders specific to the robot (i.e. nugus1, nugus2, nugus3). More on the configuration system can be found on the [Configuration and Script System](/system/nubots/config-script) page.
