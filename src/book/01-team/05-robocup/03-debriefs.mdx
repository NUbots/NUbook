...
### What Went Well
- Need to choose a good computer to be the dedicated building/commiting/pushing computer. Needs to be kept up to date so build times remain low and so it is always in a good state to make a new submission. 
WE WON A GAME!!!
Quarter finalist
Scored a goal!
Walking
Walking pretty well tuned
Ball tracking/path following
Vision
WOO GO ALEX!!!! Trained the mesh and made a real particle filter and did many other things 
Could use vision data to find and walk towards the ball
Head behaviour stopped fixating on missing objects
...
Ball detector looked like it worked well!! I (Ysobel) didn’t see any false positives. Uncommon false negatives.
Odometry
...
Fixed issue in head behaviour
NUClear
Yay NUClear!!
Webots + GameController
We now have a simulator!!!!!!!
Fixed up game controller message
We now!!!! Have!!! A!!!! simulator!!!!
DevOps and Code Review
Incremental submissions to prevent a final rush that may introduce improperly tested issues
Add info on how to test PR, what PR is about, ...
Tools: NUsight, NUbook
Identified new concepts to include within NUbook based on junior team member code reviews 
The answer is everything! ALL the documentation! 
More info in the code review page on basic things to look for (spelling, enough comments, initialising variables, additions to NUbook…)
Localisation page (there is a PR somewhere, probably needs rewritten) [Abi]
Mathematics page expansion? ‘Unit vector thing’. Letters meaning (this requires an update to NUbots too because we have double ups), have a table of the common variable names and what they mean.
Odometry
Continue logging guide (I have a branch [Ysobel])
Trent needs to explain field space, torso space, and world space and why certain things are tracked in the spaces that they are
Team Morale, Participation, and New Members
Introduced international meme review
Had impactful work done by newer members
Planning, Food, and Organisation
World record for most Red bulls crimson cows consumed 
Good idea to book the room, was good for watching matches
...
### What's next
Having a way to rapidly prototype ideas (i.e. python modules)
E.g. when developing what is now `webots.cpp`
...
Improved walk engine - Tom’s FYP work looks promising - what about flex?
...
Multimodal CycleGAN 
Motion capture data
All new vision members need to be able to train, run, test visual mesh (with dummy dataset)
Consider more goal throw outs? Still need to do green horizon throw outs. 
Optimise odometry values, motion capture set up?
Add in goal looking head behaviour
Proper state machine for behaviour, handle all cases. Kick-off in normal. Ball drop. Other things. Infrastructure to begin with rather than actual implementation of what to do in that stage.
Implement isolated tests in Webot for modules.
More NUbook pages!!
We need the director.
ROS bridge. Has a huge number of libraries that we can use if we can interface with ROS
Investigate PlotJuggler to be used as potential alternative to NUsight charts
Numerical optimization for IK (or someone do a phd on finding the analytical solution to our lower body IK)
...
Diagram visualization of our behaviour. Auto generated would be neat
Develop a simple program that uses sliders for tuning YAML files
...
Multi-state walk engine, which starts in a slower state, then once it has speed it goes much faster - i.e. what Bit-bots do
Behaviour visualisation would be cool. Either some indication in existing NUsight views of the state of the robot, or a new NUsight behaviour view, with some indication of graphs of previous states and possible future states. I can even imagine a floating box above a robot in webots indicating  its behavioural state. That would be insane, but probably way too much work lol 
Trent is suggesting a different NUsight view for different providers e.g. a NUsight view for the walk engine behaviour provider
Having time as an explicit feature. Graphs are lit
Sync instead of single in GameController.cpp - Single can drop messages, but Sync won’t
Remove if in ball detector that prevents us knowing there are no balls
Different config for our playing role. We debug and test on one role, and play on the role for playing - to prevent breaking config changes like UDP filter
