...
- Goal detector had issues and required a significant amount of post-vision processing
...
- Consider more post-vision processing including work on green horizon throw outs.
...
- Change `GameController.cpp` to use `Single` instead of `Sync` - `Single` can drop messages, but `Sync` will not
- Fix `BallDetector` to properly let the system know if it can detect no balls
- Set up a different config for our playing role, so we debug and test on one role, and play on the role for playing - to prevent breaking config changes like the network traffic filter
