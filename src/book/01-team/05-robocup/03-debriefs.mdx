...
- Issues with connecting to GameController during games

#### Vision

- Red camera. Possible hardware issue? Swapping the cable fixed the issue, but this may have reset the automatic white balance
...
- Theta was in check-in, not carry-on, so couldn’t collect data until later. Did get one image earlier using a theta from CIT Brains
...
- The robots were able to walk stabily!!
- Getups basically worked flawlessly

#### Networking

- Able to connect to GameController for most of the games
...
- Find a replacement for CM740’s quick so we can buy or make them before the end of the year.
  - Redesign NUsense to only contain the core functionality – then additional functionality can be added with shields – modular system – modules that you can plug in
- Get a decent battery powered soldering iron (TS100), a decent stand, sponge, and safety glasses with some back up wire and various connections
...
- More out of the box / off the shelf solutions for hardware, such as a proper cooling system that we can buy off the shelf
...
- Sort out general connectivity to NUsight - had issues connecting at all, possibly use a different wifi solution
