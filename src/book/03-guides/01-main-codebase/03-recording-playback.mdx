...
The following steps detail how to record data. An example will be given throughout showing how to record sensors messages.
...
2. Set up the data logging configuration file. This involves making changes to [`DataLogging.yaml`](https://github.com/NUbots/NUbots/tree/master/module/support/logging/DataLogging/data/config). Add in the messages you want to log, and set their values to `true`.
...
4. SSH into the robot and run the binary built for the role you want to record. For our example, run `./sensortest`.
...
6. Move the newly created NBS file to a computer for safe storage.
   ...
   2. Copy the file across. On the destination computer, run:
	  ...
	  The file will be copied from the robot to the folder you ran the command in. The [`scp`](https://linux.die.net/man/1/scp) command is like the regular [`cp`](https://linux.die.net/man/1/cp) command, but it copies to or from another computer over SSH. 
...
1. Remove any modules that emit the message you recorded. In this example, remove the SensorFilter module, since it emits `Sensors`. To do so, remove `platform::darwin::SensorFilter` from the role.
