...
1. Set up the [role](https://nubook.netlify.app/system/foundations/nuclear#nuclear-roles). Most of the time this involves making minor changes to an existing role.
...
2. Set up the configuration file. This involves making changes to the [`DataLogging.yaml`](https://github.com/NUbots/NUbots/tree/master/module/support/logging/DataLogging/data/config) file. Add in the message you want to log, specifying `true`.
   ...
   If you would like to log a different message:
...
1. Find the `.proto` file that defines that message, such as [`Sensors.proto`](https://github.com/NUbots/NUbots/blob/master/shared/message/input/Sensors.proto).
   ...
   You can enable multiple messages at once.
...
Messages can be emitted from an NBS file. After recording data in an NBS file, you can play it back to the system. Following on from the previous example, lets now run `sensortest.role` with recorded sensor data to analyse the results more closely off the robot.

1. Remove any modules that emit the message you recorded. In this example, remove the Sensors message. That is, remove `platform::darwin::SensorFilter` from the role.
...
5. Recompile and run the role you are playing back the data with. In the example, rerun `./b configure` and `./b build`. Run `./b run sensortest` to locally run the role.
6. For this example, it makes sense to view the robot in NUsight. Find out how to set up NUsight on the [NUbook Getting Started page](https://nubook.nubots.net/guides/main/getting-started#get-build-and-run-nusight).
