...
<<<<<<< HEAD:src/book/03-guides/01-main-codebase/03-camera-calibration.mdx
    Next you need to figure out which serial number is left and which is right. Change the serial number for one camera to an incorrect number in the Camera configuration file (this ensures it's not found when running cameratest). Run `./cameratest` and test which camera is working by covering one camera and seeing the resulting image in the NUsight vision tab. Alternatively, you could unplug one camera rather than change the serial number. If the serial number is on the wrong camera configuration file, switch them.
=======
...
>>>>>>> main:src/book/03-guides/01-main-codebase/06-camera-calibration.mdx
