...
Camera calibration is done to determine optimal parameters for the cameras, such as focal length and the transform from one camera to the other. More on the perameters being optimised for camera calibration can be found on [the input page](/system/subsystems/input).
