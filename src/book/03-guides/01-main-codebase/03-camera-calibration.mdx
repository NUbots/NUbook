...
1.  Change [`cameratest.role`](https://github.com/NUbots/NUbots/blob/master/roles/cameratest.role) to include `support::logging::DataLogging`
...
4.  Ensure the Left and Right camera serial numbers are correct in their respective configuration files (`Left.yaml` and `Right.yaml`).
    ...
    Next you need to figure out which serial number is left and which is right. Change the serial number for one camera to an incorrect number (this ensures it's not found when running cameratest). Enable compressed images in `NetworkForwarder.yaml`. Run `./cameratest` and test which camera is working by covering one camera and seeing the resulting image in the NUsight vision tab. Alternatively, you could unplug one camera rather than change the serial number. If the serial number is on the wrong camera configuration file, switch them.
...
3.  Verify these values by running vision and checking the output in NUsight.
    ...
    1. Ensure `NetworkForwarder.yaml` has enabled compressed images and vision objects.
