...
Camera calibration is done to determine optimal parameters for the cameras, such as focal length and the transform from one camera to the other. More on the parameters being optimised for camera calibration can be found on [the input page](/system/subsystems/input#cameras).
...
4.  Ensure the Left and Right camera serial numbers are correct in their respective configuration files (`Left.yaml` and `Right.yaml`).
    ...
    Next you will need to figure out which is left and which is right. Change the serial number for one camera to an incorrect number (this will ensure it is not found when running cameratest). Enable compressed images in `NUsight.yaml`. Run `./cameratest` and test which camera is working, by covering one camera and seeing the resulting image in the NUsight vision tab. Alternatively you could unplug one camera rather than change the serial number. If the serial number is on the wrong camera configuration file, switch them.
...
8.  Stop the binary and copy the nbs file created into the NUbots directory on your computer.
    ...
    2. Discover the name of the log file created. The file will be in the `log/cameratest/` folder on the robot. The name will correspond to the date and time it was created, according to the robot. Run `ls log/cameratest/` to list all logs.

    3. (optional) Rename the log file so it is easier to copy across. The name can be changed with the command `mv <old path> <new path>`.
...
2.  Calibrate the cameras by running
    ...
    The calibration routine will change the values in the camera configuration files.
...
4.  Commit these changes in a new branch on GitHub to ensure they are not lost. If you are calibrating cameras on other robots, repeat the process with them. Once the calibration is complete, commit all changes and push to the branch. Make a pull request to get the changes into the master branch. Read the [Git guide](/guides/general/git) if you are unsure about this step.
