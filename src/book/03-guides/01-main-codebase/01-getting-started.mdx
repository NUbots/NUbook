...
If this fails, run `sudo apt-get update && sudo apt-get upgrade`.
...
If your computer struggles to build and crashes, do './b build -jx', where x is the number of cores you want to use. A good recommended amount of cores is five, if you still struggle, perhaps use one core.
