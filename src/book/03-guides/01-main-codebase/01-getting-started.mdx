...
2. With the target platform selected, choose the binaries (called "roles") that you want to build. You do this by running the following command:
   ...
   If you are following the keyboardwalkfake example, you will need to make sure `keyboardwalkfake` is set to `true`. Find the list of items that are prefixed with `ROLE_` and set all to `false` except for `keyboardwalkfake`, which should be `true`. You do not need to change any other build options in this example.
...
```bash
./b edit config/NUsight.yaml
```

Set the messages you would like NUsight to receive to 'true'. For our example, you need `sensor_data` set to true.
...
- To make the robot walk, go into the terminal that is running `keyboardwalkfake` and hit <kbd>e</kbd> to enable walking. Use <kbd>w</kbd>, <kbd>a</kbd>, <kbd>s</kbd> and <kbd>d</kbd> to make the robot move.
