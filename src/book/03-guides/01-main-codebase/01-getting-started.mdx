...
1. Add Docker's stable repository.
   ...
   <Alert type='info'>
      If you are not using the latest Ubuntu LTS, you will need to change this command. The latest Ubuntu LTS is Focal Fossa, hence the word `focal` in the command. If, for example, you are using Bionic Beaver, use `bionic` in place of `focal`.

   The latest Linux Mint version, Ulyana, is built off Ubuntu Focal Fossa, therefore you would use `focal`. If you are using an old version of Linux Mint, you can find out what Ubuntu LTS it uses on the [Linux Mint all downloads page](https://www.linuxmint.com/download_all.php). For example, if you are using Linux Mint Tricia, use `bionic`.
   </Alert>

2. Add Docker's stable repository and update the apt package index.
