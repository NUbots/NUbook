...
4. Ensure Ubuntu is using WSL 2.
   ...
   <Alert type='info'>
     {' '}
     Note that 'ubuntu' in the last command may need to be capitalised. Check the
     exact spelling from the list output in the first command of this step.{' '}
   </Alert>
...
<Alert type='info'>
  The commands in the rest of this guide should be run in the WSL Ubuntu
  terminal except where specified to run them elsewhere, such as on a real
  robot.
</Alert>
...
<Alert type='warn'>
  When running on a real robot, make sure you understand how to safely handle a
  robot. To make the robot's servos relax, hit the red button on the back of the
  robot. This is an emergency stop button to prevent harm.
</Alert>
...
4. Run the following command to run NUsight:
   ...
   If you are running a NUbots binary on a port other than the default (usually by changing the `port` configuration value in config/NUClearNet.yaml), use the following command instead:

   ```bash
   ./b yarn prod --address <ADDRESS> --port <PORT>
   ```
