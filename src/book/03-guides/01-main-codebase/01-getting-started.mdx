...
Windows users who want to connect to NUsight will need to set up the docker network and run
...
6. Find the IP address of the Docker container. Run
   ...
   There will be an entry called `docker`. Find the address after the word `broadcast`. We will refer to this address as `<ADDRESS>`.
