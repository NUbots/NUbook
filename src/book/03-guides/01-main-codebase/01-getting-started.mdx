...
2. Change into the cloned directory. We use [Poetry](https://python-poetry.org/) to manage Python dependencies. Install Poetry using the official installer:

   ```bash
   cd NUbots
   curl -sSL https://install.python-poetry.org | python3 -
   ```
   This command downloads and executes the official installation script for Poetry. For more details on installing Poetry, see the [official documentation](https://python-poetry.org/docs/#installing-with-the-official-installer). Once Poetry is installed, the build system (`./b`) will use it to handle Python dependencies specified in the project.
