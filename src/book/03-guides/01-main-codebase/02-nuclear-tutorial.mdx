...
Setting up the codebase is vital! Most new members will have done this during the recruitment tasks, but if you did not do this or your setup has disappeared head over to [the Getting Started page](/guides/main/getting-started).
...
Ah-ha! If you look in the `module` folder, you will see a new folder called `TestModule`!
...
You'll see an interface appear. There will be a long list of entries starting with `ROLE` set to either `ON` or `OFF`. Arrow key down to those lines and press <kbd>Enter</kbd> to toggle the role on or off. Turn everything off except for `ROLE_testprogram`.
...
But - one tip before moving on - hit <kbd>Ctrl</kbd> + <kbd>C</kbd> to stop the program!

### Configuration

Let's take a look at the configuration file in our module. Open `module/TestModule/data/config/TestModule.yaml`.

This file will store any of values for the program that might change. It is bad practice to hardcode values, and configuration files prevent you from having to do that. Another great thing about configuration files is that they can be changed while a program is running, and that change will be reflected in the program!

Open `module/TestModule/src/TestModule.cpp`. There is one `on` statement here, the Configuration `on` statement. The code inside this function will run whenever the configuration file `TestModule.yaml` is changed and on startup.

Inside this statement, after line 13 where the log_level is set, add the following code
...
`log` will print to the terminal. The log level set from the configuration file determines what level logs are printed. The log level is currently on `DEBUG`. Lets compile and run the program again.
...
Remove all of the log statements. We will be making a 'ping-pong' program that sends an incrementing count between reactors. The increment size will be a configuration value. Let's add that configuration value now.
...
Next we need to read it in the module. Go to `TestModule.hpp`. We will make a variable to store this in. Inside the config struct on line 11, add in

```cpp
int increment = 0;
```

Always initialise your variables. Save and go to `TestModule.cpp`. Add in the following lines to read the configuration value and print a message to let the user know the increment size.
...
Up at the top of the file, include the protobuf message we just created. Note that these messages are compiled into C++ code, so include the compiled header.
...
If this works, you can move onto the next section where we make our reactors.

### Reactors

Reactors are chunks of code that will run given certain conditions are true. We will make two reactors, one that runs reactions whenever it gets a `Pong` message, and the other when it gets a `Ping` message.

Head back to `TestModule.cpp`. Underneath the Configuration reactor, add in the following reactor
...
`Trigger` means that it will run when it a `Ping` message is available. We get access to this message via the `ping` parameter. Let's add in the other reactor.
...
Next, we want to get the count from the `Ping` or `Pong` message, increment it, and send a new message with the incremented count. Here's the code for the first reactor.
...
Ok, so how do we start the reaction? There are a few ways it could be done, but we will use the `Startup` NUClear DSL. This is another reactor that will run only when the program starts up.
...
That was a bit too fast! Stop the program if you haven't already. Let's review our reactors. There's nothing stopping them from running as fast as they can, they will keep sending messages to each other as fast as possible!

Here's where the `Every` DSL word comes in. Let's replace the `Trigger<Ping>` with `Every<2, std::chrono::seconds>, With<Ping>`. This will prevent the reactor from running faster than once every two seconds. Compile and run again.
...
You should see the Ping and Pong logs running at a reasonable speed. This is the end of this tutorial. If you want to learn more, play around with some of the other NUClear DSL words and see how they work.

If you had any problems setting up the program, you can find all [the code files here](https://drive.google.com/file/d/1VmOz7A_Tfom1NtPSsO319wZmYBCv_a_I/).
