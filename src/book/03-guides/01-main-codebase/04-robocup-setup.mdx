...
10. Reboot the robot `reboot` or restart the services `sudo systemctl daemon-reload`,
    `sudo systemctl restart systemd-networkd` and `sudo systemctl restart wpa_supplicant@wlp58s0`.
