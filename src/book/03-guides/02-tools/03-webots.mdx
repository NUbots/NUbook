...
3. Ensure the IP and ports are set up correctly. If you are running everything locally on the one computer, the default settings should work. If Webots and the robot code are run on separate computers, do the following
   ...
   4. Under `red: hosts:` add the hostname of the computer you are running the robot code on. 
...
5. Run `./b run webotsrobocup` on the computer you want to run the robot code on.
...
3. Ensure the IP and ports are set up correctly. If you are running everything locally on the one computer, the default settings should work. If Webots and the robot codes are run on separate computers, do the following

   1. As done previously, get the hostname of each computer by running `hostname -I` in the terminal.

   2. On each computer you will be running the robot code on, run `./b edit config/webots.yaml` and change the IP in `server_address` to the hostname of the Webots simulator computer found in the previous step.

   3. On the computer you will be running the Webots simulator on, open the `NUWebots` repo in a text editor. Open the file `webots/projects/samples/contests/robocup/controllers/referee/game.json` (you can use <kbd>Ctrl</kbd>+<kbd>p</kbd>, search for `game.json` and the first file should be the correct one). 

   4. Under `red: hosts:` add the hostnames of the computers you are running robot code on. 

4. On each computer running robot code, run `./b webots build webotsrobocup` once to build the container with the `webotsrobocup` role.

5. Start up Webots.

6. Run `docker container run --rm --network=host -e ROBOCUP_ROBOT_ID=<ROBOT_ID> -e ROBOCUP_TEAM_COLOR=<TEAM_COLOUR> -e ROBOCUP_SIMULATOR_ADDR=<WEBOTS_IP>:<ROBOT_PORT> robocup-vhsc-nubots:robocup2021 webotsrobocup` 
