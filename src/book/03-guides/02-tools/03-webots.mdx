...
2. Set up the game configuration. 
   ...
   2. Change the `"blue":` `"id"` to `12`. 
   3. Change the `"blue":` `config:` field to `"team_1.json"`. 

   At the moment, there is an issue due to the fact that both teams have the same team ID since they are both NUbots teams. A penalty to one robot will also penalise the corresponding robot on the other team. 
...
5. Adjust the view in Webots as that the whole field is visible.
...
6. Click the HTML5 animation record button in Webots, as shown in the image below. This will record an interactive version of the game.
   ...
   ![Screenshot of webots emphasising the HTML5 animation record button.](./images/webots-record.png 'creenshot of webots emphasising the HTML5 animation record button.')


7. Click the button directly to the left of the HTML5 animation button to simulatiously record a standard video of the game.
