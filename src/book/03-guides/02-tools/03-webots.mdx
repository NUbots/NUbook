...
3. After you do the clone step below, move the patch file `webots_usb.patch` into your webots install folder, and run `git apply webots_usb.patch` inside that folder to apply it. After that, you should be able to `make -j$(nproc)` as usual.
