...
3. Run the following to open Webots with the Robocup world and the GameController:
   ...
   Sometimes closing Webots doesn't properly close the GameController subprocess. When this happens you'll get an error about GameController when next launch Webots. To fix, run the following command which terminates the GameController process, then start Webots again:
