...
To run on GPU, you need to ensure that your computer has the correct GPU drivers installed. For example, on an NVIDIA GPU `nvidia-smi` will tell you the driver you are using.
...
2. Download the [resources.zip](http://10.1.0.223:8080/share.cgi?ssid=05GE7Tx) file from the NUbots NAS and copy the resources directory from it into the NUpbr root directory.
