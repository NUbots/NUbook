...
In order to succesfully record a dataset using our Motion Capture System, is it assumed you are familiar with:
- Running a binary on the robot.
- Using the `Data Logging` module t
...
Now that we have completed the necessary steps to ensure we recieve accurate data, we are now ready to create our first **rigid body**. In our case, we can define a rigid body as *a physical object with fixed markers whose relative distances remain constant, ensuring it doesn't change shape during movement*. 
