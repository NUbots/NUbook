...
8. Once that script has finished, you must run one more command.
...
These 5 files need to be both renamed (replacing `eth0` or `wlp58s0` with the new interface name) and the contents of the files need to be edited, specifically the line that says `Name=eth0` or `Name=wlp58s0`.
