...
When making a PR you can select a number of reviewers to review your code. Each reviewer will receive a notification. Ideally, choose people who may be able to understand the problem your code is trying to solve.
...
To keep PRs small and atomic you may have to split your PR into multiple sub PRs. To do this, make a new branch branch from the original PR and move over some code. Then make a separate PR targeting the original PR's branch. Make sure to mention the original PR in the new PR's description, using `#123`, where `123` is the original PR number. After completing the sub PR, merge it back into the original PR.

## Handling Review and Reviewee Etiquette
...
- When applying changes, you can go to the **Files Changed** tab and see the comments with the context of the code. Here you can also batch apply suggestions, so they only take one commit.
