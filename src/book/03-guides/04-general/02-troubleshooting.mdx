...
If you're having errors which say that there was no such value in `b.cmake_cache` such as in this snippet try running `./b configure --clean` then `./b build`.
...
If the problem persists, you probably have an extra `CMakeCache.txt` file in your NUbots directory, which is messing up the build's cache.
Fixing this issue is as simple as finding the offending file and deleting it.
You can find the file in your text editor, or by running the find command inside the NUbots directory:

```sh
find ./ -name "CMakeCache.txt"
```

Once you have found the file, delete it then run `./b configure` and `./b build`.
