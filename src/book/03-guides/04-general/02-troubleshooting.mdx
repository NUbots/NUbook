...
then, run `./b configure --clean`. This refreshes the docker volumes, removing the offending file.

### Why isn't my new file being compiled?

If you have just added a new file to the codebase, make sure first that it's part of a role. After you have added it to a role, run `./b configure`, and the build system should detect the new file and compile it, next time you run `./b build`.
