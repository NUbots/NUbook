...
A subgroup Kanban board has five columns:

- **Backlog**: for tasks that have been identified but are not prioritised to do yet
- **Selected For Development**: for tasks to be worked on next
- **In Progress**: for tasks that someone is actively working on
- **In Review**: for tasks that are waiting on GitHub approval for or hardware approval by a senior member
- **Done**: for tasks that have been completed.
  The 'done' column is automatically cleaned up after two weeks. Each board is visualised with swimlanes - distinct rows separated by Epics. A 'no Epic' swimlane can be found at the bottom for tasks that don't fit inside any Epics.

Tasks are typically assigned by the team leader or a senior team member during planning and meetings, however team members can also volunteer for tasks by assigning themselves to the task in Jira if no one else is working on it. Team members are encouraged to move their own tasks through the columns, but this can also be done by the main project managers on the team.
