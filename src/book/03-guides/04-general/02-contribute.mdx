...
Once all of the comments are resolved, the reviewers have approved the PR, and the build checks have passed (see [_Formatting and PR Checks_](#formatting-and-pr-checks)), you can **Squash and Merge**. Squashing combines every commit on the PR into a single commit so that the `main` branch history is clean and easy to follow. Once the code has been squashed and merged, it is in the codebase and you have succeeded in contributing code to NUbots.
