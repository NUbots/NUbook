...
In order to improve runtime performance on the robots, all libraries that are build as part of the toolchain (as well as the NUbots code) are compiled with optimisations targeting the CPU of the robot.
...
```sh
target=$(gcc -garch=native -Q --help=target | grep -- '-march=' | cut -f3 | head -n1)
...
The final piece of information we need is about the cache sizes

```ssh
gcc -### -E - -march=native 2>&1 | sed -r '/cc1/!d;s/(")|(^.* - )//g | grep -oP -- "--param l[12]-cache(-line|)-size=[0-9]*"'
```

Which means that the native CPU supports both `abm` and `rtm` instructions, whereas a generic `skylake` CPU does not support these.
