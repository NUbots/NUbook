...
**NOTE**
`--` is needed to prevent python from trying to process the argument itself. This is only needed if you are specifyign arguments that start with a `-`.
...
NUbots uses four different code formatters. `clang-format` is used to format C++ and protobuf files, `black` and `isort` are used to format python files, and `cmake-format` is used to format CMake files.
...
You probably don't knkow what you are doing. But why would you listen to me?
...
1. Developers branch off of `master`, creating a new branch named `<last name>/<purpose of branch>`.
...
The file `config/ExampleMoldule.yaml` must exit for every module. This file defines all of the configuration for the module.
...
Two images are created, one intended for running code on your local machine named `generic`, and another internded for running code on the robots named `nuc7i7bnh`.
...
**SPOILER**
Libraries are rarely ever well-behaved
