...
<Alert type='warning'>
  ...
  letters match. That is, the position they are going from should match the
  origin of the space they are measured in.
</Alert>

If you would like the vector only rotated, extract the rotational component of the transform and apply it to the vector. This does not hold the same restriction as above, however the spaces do still need to match.
...
If a velocity is multiplied with by time, it becomes a vector.
