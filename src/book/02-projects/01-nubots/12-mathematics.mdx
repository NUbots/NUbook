...
So far we have expressed vectors as three dimensional vectors. However, they are really four dimensional. A vector is defined as $(x, y, z, 0)$. When it is multiplied with a homogeneous transformation matrix, it will only be rotated. A point is expressed as $(x, y, z, 1)$. When it is multiplied with a homogeneous transformation matrix, it will be rotated and then translated.

In the codebase, vectors (written as `rABb`) are dealt with as if they are points. This means they will be rotated and translated when multiplied by a homogeneous transformation matrix. To use them as true vectors, multiply the vector by the rotation matrix. How to extract the rotation matrix from a homogeneous matrix is explained below in the **Homogeneous Transformations** section.
