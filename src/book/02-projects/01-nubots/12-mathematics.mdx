...
Any vector $\mathbf{u}$ can be expressed as a linear combination of the elements of a basis:

$$
\mathbf{u} = u_1^a \mathbf{a_1} + u_2^a \mathbf{a_2} + u_3^a \mathbf{a_3}
$$
...
**Rotations** are defined by a 3x3 matrix. If we have a vector $\mathbf{u}^a$, that is the vector $u$ in the coordinate system $a$, the rotation
...
will rotate $\mathbf{u}^a$ by $\psi$ around the coordinate axis $\mathbf{a_3}$.
...
If we have a vector $\mathbf{u}$ (which goes from point C to point D) in a space $a$ and we want to rotate it from space $a$ to space $b$, we write:

$$
\mathbf{u}^b = \mathbf{R}^b_a \cdot \mathbf{u}^a
$$
...
If we have some rotation $\mathbf{R}_a^b$ and a translation component $\mathbf{u}^b$, then we have a transformation matrix that transforms from space $a$ to space $b$:

$$
\mathbf{H}_a^b = \begin{bmatrix}
\mathbf{R}_a^b & \mathbf{u}_{BA}^b \\
\mathbf{0} & 1
\end{bmatrix}
$$
