...
This guide assumes you know a few basic shell commands, and the basics of git, such as adding, committing, and pushing to a repository.

## Getting Code Into a Repository

Let's suppose you have created a file, you have made which you would like to add to a NUbots repository, called `my_file.cpp`, which is part of your project to make the robot stand up.
...
Once all of the comments are resolved, the reviewers have approved the PR, and the build checks have passed, you can `Squash and Merge`. Squashing removes detailed commit descriptions, so that `master` is not messy with every commit ever. Once the code has been squashed and merged, it is in the codebase and you have succeeded in contributing code to NUbots.
